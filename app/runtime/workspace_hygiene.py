...
def _category_for(path: Path, config: BotConfig) -> tuple[str, bool, str]:
    name = path.name
    try:
...
def _candidate_entries(config: BotConfig, request: WorkspaceUsageRequest) -> list[WorkspaceCleanupEntryRecord]:
    ...
    for root in roots_to_scan:
        ...
        root_category, root_safe, root_reason = _category_for(resolved_root, config)
        ...
        for current, dirs, _files in os.walk(resolved_root):
            ...
            for dirname in list(dirs):
                ...
                category, safe, reason = _category_for(candidate, config)
                if category == "unknown":
                    continue
