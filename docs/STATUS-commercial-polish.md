...
> **Latest change (2026-03-13):** **Bucket B follow-up — Trust- and admin-aware help.** `/start` and `/help` now reflect the current user's usable command surface. **Seams:** (1) Replaced static `HELP_TEMPLATE` with `_help_command_lines(user)` and `_build_main_help(user)` in `app/telegram_handlers.py`; both use existing `is_public_user(user)` and `is_admin(user)`. (2) Public users no longer see `/project` or `/policy` in help/start (those commands deny them via `_public_guard`). (3) Non-admin users no longer see `/admin sessions` in the main list. (4) Trusted and admin users still see the full relevant set. **Tests:** `test_help_and_start_public_user_excludes_project_and_policy` (public: no /project, /policy; /settings and /session visible); `test_help_and_start_non_admin_excludes_admin_sessions`; `test_help_and_start_admin_sees_admin_sessions_and_trusted_commands`. Existing Bucket B tests unchanged (trusted user 42 still sees /project; registration/README parity). 73 handler + README command tests pass. Next: Bucket C.
>
...
**Current:** Bucket B done. Next: Bucket C (no-op / already-handled / busy / wrong-user clarity).
