...
| 15 | Per-project defaults and behavior extensions | In progress | Slice 1 shipped: ProjectBinding extended with file_policy/model_profile; BOT_PROJECTS `|` separator; resolve_execution_context inheritance (session > project > global); `/policy inherit` and `/model inherit` to return to project defaults; phantom profile display fix; handler display parity (1086 passed, 54 skipped). |
