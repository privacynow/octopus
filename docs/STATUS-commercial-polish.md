...
> **Latest change (2026-03-11):** Phase 11 cleanup sealed — one claim path, one insert path, repository-shape tests.
> **Transport repository shape:** Single claim path: `_claim_queued_item(conn, *, item_id, worker_id, has_other_claimed_for_chat, event_name)` does load → validate queued → machine → exact CAS UPDATE → reread on 0 → classify already_handled vs corruption. `claim_for_update`, `claim_next`, and `claim_next_any` all use it. Single insert path: `_insert_initial_work_item(conn, *, item_id, chat_id, update_id, worker_id, created_at)` defaults to queued, runs claim_inline on synthetic model only when worker_id and no other claimed; `record_and_enqueue` and `enqueue_work_item` both use it. No transport mutation helper open-codes its own load–check–update or CAS/reread classification.
> **Repository-shape tests:** `test_record_and_enqueue_worker_id_none_inserts_queued`; `test_record_and_enqueue_worker_id_claimed_only_when_no_other_claimed`; `test_enqueue_work_item_matches_record_and_enqueue_initial_state`; `test_claim_for_update_exact_row_path_already_handled_when_state_changed`; `test_claim_next_returns_none_when_another_worker_claimed`; `test_shared_claim_helper_raises_corruption_when_reread_still_queued`. Existing exact CAS, dispatch corruption, and meta schema_version tests retained.
> **Transaction and invariant fixes:** `record_and_enqueue()` now rolls back on any non-`IntegrityError` (e.g. `TransportStateCorruption` from `_insert_initial_work_item`), so the connection is never left in an open transaction and the atomic record+enqueue contract holds. `_assert_no_invalid_rows_for_chat()` enforces at most one claimed row per chat (raises `TransportStateCorruption` when `claimed > 1`). Fresh schema includes `CREATE UNIQUE INDEX idx_one_claimed_per_chat ON work_items(chat_id) WHERE state = 'claimed'`. Tests: `test_record_and_enqueue_rollback_on_non_integrity_error`, `test_assert_no_invalid_rows_raises_when_two_claimed_in_chat`, `test_fresh_schema_has_one_claimed_per_chat_index`.
> **Pending request:** Unused `user_message_key` removed from `PendingRequestTransitionResult`. Phase 11 is fully sealed; next is Phase 12 (Postgres).
>
...
| 11 | Workflow ownership extraction | Done | Transport: one shared claim helper (`_claim_queued_item`), one shared insert helper (`_insert_initial_work_item`); no open-coded CAS in mutation helpers. Pending approval/retry machine; invalidation in machine. Repository-shape and regression tests in place. Phase 11 sealed. |
