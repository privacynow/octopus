...
>
> **Phase 14 (third slice, in progress) — command-specific actionability.** (1) **Objective:** Make /settings, /project, /model, and /session tell users what they can do next, not just show current state. (2) **user_messages.py:** New hints: `settings_use_buttons_hint`, `project_use_buttons_or_list_hint`, `project_list_discover_hint`, `model_choose_profile_hint`, `session_control_surface_hint_public`, `session_control_surface_hint_trusted`. (3) **Handlers:** cmd_settings appends settings_use_buttons_hint; cmd_project when no projects configured replies with no_projects_configured() (no /project list dead end), else appends buttons-or-list or list-discover hint; cmd_model appends model_choose_profile_hint when profile buttons exist; cmd_session appends trust-aware control-surface footer. (4) **Follow-up:** Session hints narrowed to "change chat settings" (not "these values"); default /project with empty cfg.projects returns no_projects_configured() and does not point at /project list. (5) **Tests:** test_settings_command_shows_current_values; test_project_includes_next_step_hint, test_project_no_projects_shows_no_projects_configured (pins no_projects_configured, rejects list hint); test_model_includes_choose_profile_hint; test_session_* assert "change chat settings" and public no /project. test_handlers (90 passed). Phase 14 remains in progress.
>
