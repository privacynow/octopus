...
> **Current state (2026-03-14) — Milestone E (Bucket E) in progress.** E is not complete. **Remaining:** Operator must run **Compose E2E** to completion (`E2E_COMPOSE=1 .venv/bin/python -m pytest tests/e2e/test_compose_flows.py -v -n 0`) where Docker is available; only after a full E2E pass (or a documented blocker) can E be marked complete.
>
> **This pass (2026-03-14) — implemented and verified:** (1) **Setting-family owner collapse:** One mutator per family (`_apply_model_change`, `_apply_project_change`, `_apply_policy_change`, `_apply_approval_change`, `_apply_compact_change`) and one row/button helper per family (`_settings_model_buttons`, `_settings_project_buttons`, `_settings_policy_buttons`, `_settings_compact_buttons`, `_settings_approval_buttons`) in telegram_handlers. cmd_settings, cmd_project, cmd_policy, cmd_compact, cmd_approval, cmd_model and handle_settings_callback now use them; no open-coded duplicate rows or mutation paths. (2) **Test rehome:** Eight handler-surface tests moved from test_request_flow to test_handlers: test_session_command_shows_public_context, test_settings_command_public_user_no_trusted_leak, test_settings_command_public_user_keyboard_no_project_or_policy, test_model_command_public_user_can_switch_to_allowed_profile, test_model_command_public_user_rejected_for_restricted_profile, test_model_callback_public_user_rejected_for_restricted_profile, test_model_callback_public_user_allowed_for_available_profile, test_project_callback_public_user_denied. test_request_flow module docstring updated: that file owns execution context, trust shaping, invalidation, request-lifecycle contracts; test_handlers owns user-visible command/callback surfaces. (3) **Handler test setup:** `public_user_config_overrides(**extra)` added in tests/support/handler_support.py; re-homed and several existing public-user tests now use `fresh_env(config_overrides=public_user_config_overrides(...))` instead of repeated fresh_data_dir + make_config + setup_globals. (4) **Suites run:** test_handlers (83), test_request_flow (44), test_handlers_approval, test_workitem_integration, test_user_messages, test_operator_scripts — all passed. test_config has 3 pre-existing failures (asyncio/polling, unrelated to this pass). (5) **Compose E2E:** Run started; test_compose_postgres_up_without_env_bot, test_compose_bootstrap_doctor, test_compose_db_update_smoke passed; run timed out before remaining tests completed. Full E2E must be run by operator to completion before closing E.
>
> **Follow-up (review):** Model-profile selection single contract. One canonical denial in user_messages: `trust_model_profile_not_available(profile, available)`; removed `trust_unknown_profile_available` and `trust_unknown_or_restricted_profile`. One selection helper in telegram_handlers: `_apply_model_selection(chat_id, session, profile, cfg, trust_tier)` returns (ok, message); both cmd_model and handle_settings_callback model branch use it. Re-homed model/project tests strengthened to assert exact canonical messages (reply text / edit_text); added test_model_command_and_callback_same_denial_contract and test_model_command_and_callback_same_success_contract. test_handlers 85, test_user_messages, test_request_flow passed.
>
> **Landed earlier for E:** README dev_up/doctor; lib_env.sh and script consolidation; telegram_handlers: 7 unused imports and _project_working_dir removed, 8 user_messages wired, _settings_model_profile_state used in cmd_session/cmd_settings/cmd_model/callback; E2E single bot_image_built and timeouts; test fixes and test_settings_and_admin_messages_bucket_e.
>
