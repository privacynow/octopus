...
>
> **Phase 14 (third slice, in progress) — command-specific actionability.** (1) **Objective:** Make /settings, /project, /model, and /session tell users what they can do next, not just show current state. (2) **user_messages.py:** New hints: `settings_use_buttons_hint`, `project_use_buttons_or_list_hint`, `project_list_discover_hint`, `model_choose_profile_hint`, `session_control_surface_hint_public`, `session_control_surface_hint_trusted`. (3) **Handlers:** cmd_settings appends settings_use_buttons_hint; cmd_project appends buttons-or-list or list-discover hint depending on whether project buttons exist; cmd_model appends model_choose_profile_hint when profile buttons exist; cmd_session appends trust-aware control-surface footer (public: /settings or /model only; trusted: /settings, /project, /model). (4) **Boundaries:** Public /session must not advertise /project; trusted gets full control-surface hint. (5) **Tests:** test_settings_command_shows_current_values asserts settings_use_buttons_hint; test_project_includes_next_step_hint, test_project_no_projects_includes_list_hint; test_model_includes_choose_profile_hint; test_session_includes_control_surface_hint_trusted, test_session_control_surface_hint_public_no_project. test_handlers (90 passed), test_request_flow (44 passed). Phase 14 remains in progress.
>
