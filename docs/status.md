...
>
> **Phase 20 current implementation (2026-03-16).** **Phase 20 is now the active roadmap phase.** Current milestone truth: **M1 complete**, **M2 complete**, **M3 complete**, **M4 complete**, **M5 complete**, **M6 complete**, **M7 complete**, **M8 complete**, **M9 complete**, **M10 in progress**. Registry foundation is live: typed agent/conversation/routed-work models, async registry client, durable agent runtime state, FastAPI registry service with SQLite and Postgres backend parity, delivery queue, discovery search, routed-task APIs, and a richer HTML/CSS/JS UI surface. Registry polling is wired into the same local worker/state-machine core used by Telegram; there is no parallel execution branch. Surface integration is live for registry-originated input/actions and routed tasks, including approval/reject/cancel/retry/recovery actions, chat-busy `retry_later`, routed-task result return, per-delivery poll isolation, runtime survival on unexpected poll errors, degraded-mode poll backoff with jitter, `/doctor` registry/delegation diagnostics, registry-surface timeline publication, UI-started work, UI-based delegation approval/cancel, parent-side delegation completion summaries before re-entry, authenticated Registry UI access, conversation search/filter, registry-side export, live access overrides, completion webhook notifications, skills management with enable/disable toggles, registry store SQLite/Postgres parity with abstract protocol and parameterized contract tests, and a stabilized programmatic conversation trigger API. **Latest completed Phase 20 slice:** M10I stabilized the programmatic API trigger. **Current remaining M10 scope:** usage and cost visibility (M10B).
>
