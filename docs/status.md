...
> **Phase 20 M7 active — Registry UI conversation timeline and human-initiated work (2026-03-15).** All M1–M6 milestones are complete. The full test suite now runs automatically on any Docker-enabled machine: `tests/e2e/conftest.py` auto-sets `E2E_COMPOSE=1` when Docker is available; `E2E_COMPOSE_NO_AUTODETECT=1` suppresses this for meta-test subprocess isolation. Two intentional skips remain: `test_session_db_created_on_first_use_sqlite[postgres]` (SQLite-specific feature, Postgres variant meaningfully inapplicable) and `test_compose_bot_stub_smoke` (dev/CI-only stub image, not the supported runtime path). Verification: `1261 passed, 2 skipped`. M7 is the next and final milestone for Phase 20: bot publishes timeline events via `RegistryConversationIO` lifecycle hooks, and the Registry UI gains conversation detail with timeline, plus human-initiated conversation from UI. Milestone truth: **M1–M6 complete, M7 active**.
>
...
| 20 | Networked multi-agent platform | In progress | M1 complete, M2 complete, M3 complete, M4 complete, M5 complete, M6 complete. Registry-backed mode is the product default; bots stay private and poll the public registry. Parent-side routed-result continuation re-enters the local work queue through the normal worker-owned path; `/discover`, delegation plan UX, approval-driven fan-out, degraded-mode blocking, jittered degraded polling, and `/doctor` registry diagnostics are live. Surface selection comes from `app/transports/factory.py`, while config-driven trust classification lives in `app/access.py`. |
