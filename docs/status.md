...
> **Phase 20 M9 complete — first-run polish and Registry UI UX shipped (2026-03-15).** First-run setup is now materially clearer: `container_provider_login.sh` prints explicit Claude/Codex exit banners and continues safely to the provider health check even if the provider CLI exits non-zero; `registry/start.sh` prints the enrollment token value directly; and `guided_start.sh` now supports a default quick mode, auto-starts and auto-reads local registry tokens, uses a Linux-safe default registry URL, and ends with a boxed success summary that reprints the bot's next steps. README now centers a one-command guided setup, includes a BotFather walkthrough, a "Verify it's working" section, a Registry UI section, and a live screenshot at `docs/registry-ui-screenshot.png`. The Registry UI now has system fonts, loading/refresh/freshness indicators, inline error banners, color-coded badges, instructional empty states, keyboard-accessible clickable cards, search/filter, export, and a browser-safe `/ui` login flow. Milestone truth: **M1–M9 complete, M10 next**.
>
> **Phase 20 M8 review fixes applied (2026-03-15).** The delegation completion path now avoids duplicate user messaging on crash-and-retry: if the synthetic parent continuation is already queued (`duplicate` admit), the bot no longer re-sends the raw completion summary. The completion summary itself is now explicitly framed as preliminary ("raw results while I synthesize the final answer") so the follow-on synthesized parent response reads as a continuation instead of a contradictory second conclusion. Registry UI delegation actions now clear their pending-button state immediately on success, and Telegram callback coverage now explicitly proves the `delegation_approve:{chat_id}` / `delegation_cancel:{chat_id}` path still submits or clears delegation through the shared handlers. Focused regression suites remain green on the current tree.
>
> **Phase 20 M8 complete — Registry UI delegation actions and completion flow shipped (2026-03-15).** Registry UI conversations can now approve or cancel delegation without switching to Telegram: delegation plans publish a `delegation_proposed` timeline event, the detail view renders approve/cancel controls on the live event, and `/actions` deliveries now carry `conversation_ref` so the bot routes `approve_delegation` / `cancel_delegation` through the same shared delegation handlers used by Telegram callbacks. Parent-side delegation state is now durable through completion: `PendingDelegation` has explicit `submitted`, `completed`, and `partial_failed` states; child results drive those transitions; a structured completion summary is sent to the originating surface before the synthetic parent continuation runs; and the pending delegation is only cleared after that continuation actually executes. Verification on the current tree: focused M8 suites are green, the new Compose E2E `test_compose_registry_ui_delegation_flow` is green, and the full suite with Compose E2E enabled is `1285 passed, 2 skipped`. Milestone truth: **M1–M8 complete, M9 next**.
>
> **Phase 20 M7 review hardening complete (2026-03-15).** Registry timeline/state handling was tightened after review. Conversation status no longer reopens incorrectly after a cancel request: UI-side `control` events move the conversation to `cancelling`, and late `started`/`progress` updates preserve that state until a terminal outcome arrives. Registry timeline writes are now ownership-checked against the authenticated agent, routed-task child conversations publish their own terminal `completed`/`failed` timeline event before reporting results upstream, and `RegistryConversationIO` caches the "no client available" case so degraded/not-enrolled conversations do not re-read runtime state from disk on every publish attempt. Focused regressions now cover cancel-state preservation, foreign timeline rejection, routed-task child completion, and missing-client caching. Milestone truth is unchanged: **M1–M7 complete, M8 next**.
>
> **Phase 20 M7 complete — Registry UI timeline and human-initiated work shipped (2026-03-15).** Registry-surface conversations now publish `started`, rate-limited `progress`, and `completed`/`failed` timeline events through `RegistryConversationIO` lifecycle hooks. The inline registry UI now supports a drill-down conversation detail view with timeline auto-refresh, follow-up message entry, cancel requests, connectivity-state coloring, and a "New conversation" form that targets connected bots. Registry bootstrap now includes per-conversation `timeline_event_count`, and the Compose E2E proof now covers UI-created work plus bot-side `started` publication against a real Compose-hosted registry service using the production registry client and registry surface. Verification on the current tree: `tests/test_registry_adapter.py`, `tests/test_registry_service.py`, `tests/test_handlers.py`, `tests/e2e/test_compose_flows_probe.py`, and `tests/e2e/test_compose_flows.py -k registry_ui_conversation_detail` are green; full suite result on the current tree is `1271 passed, 2 skipped`. Milestone truth: **M1–M7 complete, M8 next**.
>
> **Phase 20 M6 complete — backoff, doctor diagnostics, and docs alignment shipped (2026-03-15).** `AgentRuntime.run_forever()` now uses exponential backoff with full jitter while registry connectivity is degraded and resets immediately on reconnect, so bots stop hammering the registry during outages. `/doctor` now reports registry runtime truth instead of config-only truth: degraded connectivity, incomplete enrollment, stale last successful contact, and sessions with delegation plans still awaiting approval. README and ARCHITECTURE now describe the shipped multi-agent system directly: registry-backed mode vs standalone mode, the surface factory as the outbound dispatch decision point, the delegation flow, degraded-mode behavior, and `/doctor` as the operator surface for registry state. Verification for the current tree: `1198 passed, 65 skipped`.
>
> **Docs boundary correction (2026-03-15).** `docs/plan.md` is now trimmed
> back to roadmap, milestone, and acceptance material for Phase 20. Slice-level
> implementation detail, verification notes, and shipped-change history remain
> in this status log. Milestone truth is unchanged: **M1 complete, M2 complete,
> M3 complete, M4 complete, M5 complete, M6 next**.
>
> **Phase 20 M5 complete — delegation plan UX and routed task submission shipped (2026-03-15).** M1 Closure Gate remains sealed: `app/transports/factory.py` owns surface construction and source-aware trust classification, while `app/access.py` is the leaf-level trust module. M5 is now complete end-to-end: `/discover` is live; `PendingDelegation`/`DelegatedTask` are durable in session; `DelegatedTask.instructions` round-trips through storage; providers can return typed delegation proposals through `RunResult`; `execute_request()` persists the plan before rendering it; Telegram now shows an approve/cancel delegation keyboard; approve submits one routed task per proposed child and marks it `submitted`; cancel clears the plan without fan-out; degraded approval blocks submission and preserves state; parent-side `routed_result` continuation still re-enters the local worker path. Milestone truth: **M1 complete, M2 complete, M3 complete, M4 complete, M5 complete, M6 next**.
>
> **Phase 20 M1 closure and surface factory completion (2026-03-15).** The remaining M1 Closure Gate work is now complete. `app/transports/factory.py` is the single owner for `conversation_surface_name(conversation_ref)`, `create_outbound_surface(...)`, and the source-aware `trust_tier_for_source(..., config=...)`. `app/access.py` is now the leaf-level owner for config-driven allow/admin/public classification and trust-tier resolution, so the surface factory no longer imports `telegram_handlers`. `worker_dispatch` and `app/agents/delivery.py` no longer import or construct concrete surface adapters directly; they depend on the factory plus `InteractionSurface`, and the last stale inline registry-trust branch in the replay path was removed. `TelegramConversationIO.send_message()` now delegates to `send_text()` so replies, handles, and test oracles stay aligned. `ConversationSimulator` can now inject registry-surface messages through the same durable worker boundary, and the simulator output log captures registry-surface bot output alongside Telegram output. Acceptance checks are green: no `conversation_surface` or `_BotMessage` references remain in `worker_dispatch`, the `skip_approval` round-trip tests are live, the named registry regression tests still pass, and the full suite is green. Suite: `1181 passed, 65 skipped`.
>
> **Phase 20 current implementation (2026-03-16).** **Phase 20 is now the shipped product baseline.** Current milestone truth: **M1 complete**, **M2 complete**, **M3 complete**, **M4 complete**, **M5 complete**, **M6 complete**, **M7 complete**, **M8 complete**, **M9 complete**, **M10 complete**. Registry foundation is live: typed agent/conversation/routed-work models, async registry client, durable agent runtime state, FastAPI registry service with SQLite and Postgres backend parity, delivery queue, discovery search, routed-task APIs, and a richer HTML/CSS/JS UI surface. Registry polling is wired into the same local worker/state-machine core used by Telegram; there is no parallel execution branch. Surface integration is live for registry-originated input/actions and routed tasks, including approval/reject/cancel/retry/recovery actions, chat-busy `retry_later`, routed-task result return, per-delivery poll isolation, runtime survival on unexpected poll errors, degraded-mode poll backoff with jitter, `/doctor` registry/delegation diagnostics, registry-surface timeline publication, UI-started work, UI-based delegation approval/cancel, parent-side delegation completion summaries before re-entry, authenticated Registry UI access, conversation search/filter, registry-side export, live access overrides, completion webhook notifications, reported token usage visibility, skills management with enable/disable toggles, registry store SQLite/Postgres parity with abstract protocol and parameterized contract tests, and a stabilized programmatic conversation trigger API. **Latest completed Phase 20 slice:** M10B reported token usage visibility. **Current remaining M10 scope:** none; M10 is complete.
>
> **Historical Phase 15 baseline (superseded by Phase 20).** Transport foundation, proof-hardening, and doc alignment were already in place before the registry-backed product work accelerated. Proof-hardening: credential owner suite migrated to drain/running_worker (no inline-execution assumptions); queued-cancel durable-state proof added for both SQLite and Postgres (contract test + Postgres regression); stale production comments updated (_LIVE_CANCEL, worker.py). Transport foundation: project-owned types and ports in `app/transports/` (InboundEnvelope, ConversationIO, EditableMessageHandle, TransportCapabilities); fresh plain-message admission uses `InboundEnvelope` in production ingress; worker-owned outbound output uses `TelegramConversationIO`; handler-owned command/callback/immediate reply paths still use direct PTB message/query/chat methods. Simulator: handler-level harness in `tests/support/conversation_simulator.py` — injects via handle_message/cmd_* directly (no transport-level InboundEnvelope ingress, no callback injection yet), runs real worker, and exposes one ordered **text** output log (`reply_text`, `edit_text`, `chat.send_message`, `reply_photo`/`reply_document` caption or placeholder, bot `send_message`/`send_photo`/`send_document` and edits, callback `answer`, callback `edit_message_text`; markup-only edits not included). Canonical E2E: `tests/test_simulator_e2e.py` covers message→long run→cancel (exact message-item + command-item durable shape), cancel before claim, second message busy (zero runnable, exact `chat_busy` shape), credential reply off-queue, and recovery notice. Documentation alignment: `ARCHITECTURE.md` now describes the shipped interfaces, component boundaries, and simulator limits as architecture; `plan.md` now separates shipped Slice 5 foundation from the remaining target state; `README.md` now reflects the worker-owned runtime model and current `/cancel` semantics. (1) **Runtime matrix:** local + sqlite = default path; local + postgres = supported path; shared = out of scope. (2) **Single backend seam:** `app/runtime_backend.py` is the only backend selector; `storage.py` and `work_queue.py` are backend-neutral facades; no facade leaks of SQLite-only internals. (3) **Contract suites:** `tests/contracts/test_session_store_contract.py` and `tests/contracts/test_transport_store_contract.py` are parameterized for both SQLite and Postgres. **Note:** Postgres parameters skip when Docker/Postgres is unavailable; on environments without Docker the contract suites only exercise SQLite. (4) **Startup/operator:** Default path is SQLite Local Runtime (no `BOT_DATABASE_URL`); Docker `bot` service does not depend on Postgres; `./scripts/app/dev_up.sh` and `./scripts/app/guided_start.sh` are SQLite-first; README documents SQLite as default, Postgres as optional; troubleshooting section no longer claims Postgres-only. (5) **E2E:** Primary gate is `test_compose_sqlite_local_runtime_primary` (Docker Local Runtime with SQLite). Bounded Postgres coverage is **real**: `test_compose_bot_startup_with_postgres` uses a dedicated override that injects `BOT_DATABASE_URL=postgresql://bot:bot@postgres:5432/bot` into the bot service so the bot actually runs against Postgres. One module-scoped `bot_image_built` fixture; no duplicate image build. (6) **Probe tests:** `test_postgres_bot_override_injects_bot_database_url` and `test_postgres_bot_override_path_writes_file` assert the Postgres override contract. (7) **Tests:** Contract suites + storage, work_queue, config, handler_runtime_isolation, sqlite/workitem integration; Postgres tests remain for supported backend. **Phase 14 sealed** (operator health clarity, dead-end command fixes, command-specific actionability, command/callback parity).
>
...
>
> **Prior (2026-03-13):** Tooling independent of bot config: **bot** service under profile **bot** so `docker compose --project-directory . -f infra/compose/docker-compose.yml up -d postgres` and db-* tooling work without `.env.bot`. Bot start: **docker compose --project-directory . -f infra/compose/docker-compose.yml --profile bot --env-file .env.bot up -d bot**. **Provider-tagged images** (`telegram-agent-bot:claude`, `telegram-agent-bot:codex`); build via `./scripts/provider/build_bot_image.sh` (docker build, no compose build); `./scripts/provider/provider_login.sh` checks `docker image inspect` and fails with rebuild message if image missing. `./scripts/app/guided_start.sh` single-pass: runs provider login if needed, then starts bot (no “rerun script”). README front door: clone, `.env.bot`, `./scripts/app/guided_start.sh`. E2E: **test_compose_postgres_up_without_env_bot**; shell contract: image-inspect guard, Compose file under `infra/compose/`, and `--profile bot` in compose args.
>
...
> [plan.md](plan.md), with Postgres queue
...
- The default operator path today is **Local Runtime**, but Shared Runtime
  ...
  - Shared Runtime requires `BOT_RUNTIME_MODE=shared` plus webhook mode
  - Shared Runtime queue admission is `duplicate` / `admitted` / `queued`
  - Shared Runtime stale recovery is lease-based and replay-notice only
...
- Phase 20 milestone truth today:
  ...
  - M5 discovery/delegation is complete: `/discover`, delegation plan UX,
    routed task submission, degraded-mode approval blocking, and parent-side
    routed-result continuation are all live
  - M6 hardening/docs completion is complete: degraded polling backoff,
    registry/delegation diagnostics in `/doctor`, and operator/architecture
    docs are aligned with the shipped multi-agent system
  - M7 registry UI timeline + human-initiated work is complete: registry-surface
    conversations publish started/progress/outcome timeline events through
    `RegistryConversationIO`, registry bootstrap surfaces `timeline_event_count`,
    the UI has conversation detail + auto-refresh + follow-up input, and the
    Compose E2E proves UI-created work can reach a real registry surface and
    publish a `started` event
  - M8 delegation approval/cancel and parent-side completion flow is complete
  - M9 first-run polish and Registry UI UX is complete
  - M10 is complete:
    migrations and upgrade docs, Registry UI auth, search/filter, live access
    overrides, registry export, completion webhooks, reported usage
    visibility, skills management, the stabilized trigger API, and the
    registry/runtime Postgres parity hardening are all shipped
...
| 20 | Networked multi-agent platform | Done | M1-M10 are complete. Registry-backed mode is the product default; bots stay private and poll the public registry. Registry UI auth, search/filter, live access overrides, export, completion webhooks, reported usage visibility, skills management, stabilized programmatic trigger API, and registry/runtime Postgres parity are shipped. |
...
Phase 20 complete and Phases 16-19 deferred beyond it.
...
| Update after pull | README points users back to `./scripts/app/guided_start.sh`; that path rebuilds and restarts when rev/files changed. | — |
...
- `--doctor` (CLI) delegates to the canonical runtime-health collector in `app/runtime_health.py`.
...
- `scripts/host/setup_instance.sh` prunes codex-specific config lines when provider is claude (and vice versa).
...
[plan.md](plan.md). This section is
...
| `--doctor` crashes when data_dir doesn't exist | Medium | The shared health collector unconditionally scanned sessions via SQLite, which fails before first bot startup when `data_dir` hasn't been created yet | Added `config.data_dir.is_dir()` guard before stale session scan. Regression test covers the path. |
...
| `/doctor` and `--doctor` crash on corrupt session database | Medium | `scan_stale_sessions()` calls `list_sessions()` → `_db()` → SQLite open, which throws `DatabaseError` on junk/corrupt `sessions.db`. The health command — the one tool meant to diagnose problems — crashed instead of reporting them. | Wrapped stale session scan in the shared health collector with `sqlite3.DatabaseError`/`OperationalError` handling. Reports corruption as an error in the health report. |
| Telegram `/doctor` crashes on corrupt DB before reaching health checks | Medium | `cmd_doctor` called `_load()`, which hit SQLite before the shared health collector ran. Corrupt DB raised unhandled `DatabaseError`, user saw nothing. | `cmd_doctor` wraps `_load()` in `DatabaseError`/`OperationalError` handling; on failure, passes `session=None` to the health collector so non-session checks still run. |
| `/doctor` crashes on newer session DB schema version | Medium | `storage._db()` raises `RuntimeError` when `schema_version > supported`, but both the shared health collector and `cmd_doctor` only caught `sqlite3` exceptions. Downgrading the bot with an existing DB crashed the health command. | Added `RuntimeError` handling in both the health collector and `cmd_doctor`. Regression tests cover both CLI and Telegram paths. |
...
| `bootstrap.sh` installs dev deps in production setup | Low-medium | `scripts/app/bootstrap.sh` unconditionally installed `requirements-dev.txt` (pytest, xdist). `scripts/host/setup_instance.sh` calls bootstrap, so operator installs got test tooling. | Dev deps only installed when `BOT_SETUP_RUNNING` is unset (standalone bootstrap, not setup wizard). |
...
|-------|-----|---------------|
| Duplicated doctor logic between CLI `--doctor` and chat `/doctor` | Extracted a shared health seam first, then consolidated it into the canonical `app/runtime_health.py` report/formatter pipeline. | `app/runtime_health.py`, `app/main.py`, `app/telegram_handlers.py` |
| Session-scan queries (`scan_stale_sessions`, `check_prompt_size_cross_chat`) lived in `telegram_handlers.py` | Moved behind the shared health/skills seams so Telegram is only an adapter, not the owner. | `app/runtime_health.py`, `app/skills.py`, `app/telegram_handlers.py` |
...
| Prompt weight not shown in `/doctor` | Medium | `/session` showed character estimate but `/doctor` did not | Added prompt-weight rendering to the shared health output. |
...
| `/doctor` prompt weight used raw session, not resolved context | Medium | The shared health path computed prompt weight from `session["active_skills"]`, ignoring public trust tier stripping | Moved computation to the resolved-context path before formatting doctor output. |
...
[plan.md](plan.md). The material below is
