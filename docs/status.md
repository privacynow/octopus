...
> **Phase 20 M10 hardening follow-up — time and validation cleanup landed for review (2026-03-15).** Request-lifecycle stale checks no longer assume pending timestamps are always epoch floats: shared UTC helpers now accept both legacy float timestamps and ISO 8601 strings, so approval/setup/delegation TTL checks remain correct if durable storage shifts representation. Config validation now centralizes provider/mode/file-policy values through enums and uses `urllib.parse` for registry/database URL validation instead of string-prefix checks. Skills parsing now uses typed Pydantic validation for `requires.yaml` and provider YAML payloads, while `/doctor` stale-session scans now rehydrate `SessionState` and inspect typed pending/delegation objects instead of walking raw nested dicts. Focused request-flow, config, skills, and doctor regressions are being exercised on the current tree.

> **Phase 20 M10C/M10A in progress — migrations and Registry UI auth landed for review (2026-03-15).** `transport.db` now migrates in-place through numbered schema versions instead of treating every older version as unsupported: legacy queue databases can progress `v2 → v3 → v4 → v5`, adding `dispatch_mode`, `usage_log`, and `user_access` idempotently before validation runs. The registry SQLite store now has its own `meta` table and schema versioning, with `skills_override` introduced as registry schema `v1` so existing unversioned registry databases upgrade in place on first open. The repo also now has a root `VERSION` file (`M10`), a milestone-oriented `CHANGELOG.md`, an operator `docs/UPGRADE.md`, and guided setup reprints the repo version in its success summary. On top of that foundation, the Registry UI HTML surface is now cookie-gated through Starlette `SessionMiddleware`: `/ui` redirects to `/ui/login` without a valid session, successful login stores signed session state, `/ui/logout` clears it, and `/v1/ui/*` bearer-token auth remains unchanged. The auth slice now uses dependency-backed form parsing (`python-multipart`) and framework session signing (`itsdangerous`) instead of hand-rolled parsing or in-memory session bookkeeping. Operator-facing scripts and README now point to `/ui` plus `REGISTRY_UI_TOKEN` as the login password instead of the old query-token flow. Focused migration, auth, operator-script, README, and docs/status regressions are green on the current tree.
>
> **Phase 20 M9 in progress — first-run polish and Registry UI UX pass landed for review (2026-03-15).** First-run setup is now materially clearer: `container_provider_login.sh` prints explicit Claude/Codex exit banners and continues safely to the provider health check even if the provider CLI exits non-zero; `registry/start.sh` prints the enrollment token value directly; and `guided_start.sh` now supports a default quick mode, auto-starts and auto-reads local registry tokens, uses a Linux-safe default registry URL, and ends with a boxed success summary that reprints the bot's next steps. The latest review fix makes that summary reprint a browser-safe Registry UI URL: local registry URLs are normalized to `http://localhost:.../ui`, while external registries show `/ui` without inventing a token; in the current tree, M10A adds the follow-on login step using `REGISTRY_UI_TOKEN` as the password. README now centers a one-command guided setup, includes a BotFather walkthrough, a "Verify it's working" section, a Registry UI section, and a live screenshot at `docs/registry-ui-screenshot.png`. The inline Registry UI now has system fonts, loading/refresh/freshness indicators, inline error banners, color-coded badges, instructional empty states, keyboard-accessible clickable cards, and basic bot/task detail views in the side panel. Verification so far: shell syntax clean; `tests/test_operator_scripts.py`, `tests/test_readme_operator.py`, `tests/test_readme_commands.py`, `tests/test_registry_service.py`, and `tests/test_docs_runtime_status.py` are green; `tests/test_docker_ops.sh` is green; the live registry renders successfully and the screenshot asset was captured from the running UI. Milestone truth: **M1–M8 complete, M9 in progress**.
>
...
>
> **Phase 20 current implementation (2026-03-15).** **Phase 20 is now the active roadmap phase.** Current milestone truth: **M1 complete**, **M2 complete**, **M3 complete**, **M4 complete**, **M5 complete**, **M6 complete**, **M7 complete**, **M8 complete**, **M9 in progress**. Registry foundation is live: typed agent/conversation/routed-work models, async registry client, durable agent runtime state, FastAPI + SQLite registry service, delivery queue, discovery search, routed-task APIs, and a vanilla HTML/CSS/JS UI shell. Registry polling is wired into the same local worker/state-machine core used by Telegram; there is no parallel execution branch. Surface integration is live for registry-originated input/actions and routed tasks, including approval/reject/cancel/retry/recovery actions, chat-busy `retry_later`, routed-task result return, per-delivery poll isolation, runtime survival on unexpected poll errors, degraded-mode poll backoff with jitter, `/doctor` registry/delegation diagnostics, registry-surface timeline publication, UI-started work, UI-based delegation approval/cancel, and parent-side delegation completion summaries before re-entry. **Latest completed Phase 20 slice:** M8 closed the delegation UX loop in the Registry UI and completed the originating-surface result path. **Current slice:** M9 first-run polish and Registry UI UX improvements are in review. Verification for the current tree: focused operator/docs/registry suites are green, `tests/test_docker_ops.sh` is green, and the live registry UI renders successfully with a fresh screenshot captured from the running app.
>
...
| 20 | Networked multi-agent platform | In progress | M1 complete, M2 complete, M3 complete, M4 complete, M5 complete, M6 complete, M7 complete, M8 next. Registry-backed mode is the product default; bots stay private and poll the public registry. Parent-side routed-result continuation re-enters the local work queue through the normal worker-owned path; `/discover`, delegation plan UX, approval-driven fan-out, degraded-mode blocking, jittered degraded polling, `/doctor` registry diagnostics, registry-surface timeline publication, and registry UI conversation detail/new-work flow are live. Surface selection comes from `app/transports/factory.py`, while config-driven trust classification lives in `app/access.py`. |
