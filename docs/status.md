...
> **Phase 20 M5 Phase 2 now active — delegation plan UX and routed task submission (2026-03-15).** M1 Closure Gate is fully verified and sealed: `app/transports/factory.py` owns all surface construction and trust classification; `app/access.py` is the leaf-level trust module; no orchestration code imports concrete adapter classes; `TelegramConversationIO.send_message()` delegates to `send_text()`; suite is 1181 passed, 65 skipped. M5 Phase 1 is complete: `/discover` is live, `PendingDelegation`/`DelegatedTask` are durable in session, and the result-continuation path (routed_result → session update → synthetic re-entry) works. **Active frontier is M5 Phase 2:** delegation plan builder (`app/agents/orchestration.py`), delegation plan Telegram UX (inline keyboard with approve/cancel), approval callback that calls `client.submit_routed_task()` per `DelegatedTask`, cancellation callback, degraded-mode guard in callback path, `DelegatedTask.instructions` field with serialization round-trip, and tests for all four paths (approve, cancel, degraded-block, plan-builder). Milestone truth: **M1 complete, M2 complete, M3 complete, M4 complete, M5 Phase 1 complete, M5 Phase 2 active, M6 not started**.
>
...
>
> **Phase 20 current implementation (2026-03-15).** **Phase 20 is now the active roadmap phase.** Current milestone truth: **M1 complete**, **M2 complete**, **M3 complete**, **M4 complete**, **M5 partial**, **M6 not started**. Registry foundation is live: typed agent/conversation/routed-work models, async registry client, durable agent runtime state, FastAPI + SQLite registry service, delivery queue, discovery search, routed-task APIs, and a vanilla HTML/CSS/JS UI shell. Registry polling is wired into the same local worker/state-machine core used by Telegram; there is no parallel execution branch. Surface integration is live for registry-originated input/actions and routed tasks, including approval/reject/cancel/retry/recovery actions, chat-busy `retry_later`, routed-task result return, per-delivery poll isolation, and runtime survival on unexpected poll errors. **Latest completed Phase 20 slice:** the M1 closure refactor moved outbound surface selection into `app/transports/factory.py`, moved config-driven trust classification into `app/access.py`, finished the `InteractionSurface` hook ownership for bind/input/outcome/recovery behavior, removed the last stale inline source-based trust branch, and upgraded the simulator to inject registry-surface messages through the same worker-owned path. **Current in-progress Phase 20 slice:** `/discover` remains the first user-facing discovery command. It builds a structured `AgentDiscoveryQuery`, searches the registry by role/skills/tags/free text, renders candidates in Telegram, and reports standalone/degraded states truthfully instead of pretending discovery succeeded. Verification for the current tree: `1181 passed, 65 skipped`.
>
...
- Phase 20 milestone truth today:
  ...
  - M5 discovery/delegation is partially started: parent-side routed-result
    continuation is live; `/discover` command is live; delegation plan UX, fan-out,
    and routed task submission remain
...
| 20 | Networked multi-agent platform | In progress | M1 complete, M2 complete, M3 complete, M4 complete, M5 partial. Registry-backed mode is the product default; bots stay private and poll the public registry. Parent-side routed-result continuation re-enters the local work queue through the normal worker-owned path. Surface selection comes from `app/transports/factory.py`, while config-driven trust classification now lives in `app/access.py`; `/discover` is live. Remaining M5 scope is delegation planning, fan-out, and routed task submission. |
