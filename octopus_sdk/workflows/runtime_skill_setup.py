"""SDK-owned runtime-skill setup workflows."""
...
class RuntimeSkillSetupUseCases(RuntimeSkillSetupPort):
    ...
    def __init__(
        ...
        *,
        ...
        default_validator: CredentialValidatorPort | None = None,
    ) -> None:
    ...
    def cancel(
        ...
        *,
        ...
        decision = decide_setup_action(
            self._snapshot(session),
            CancelSetupAction(actor_key=actor_key, allow_override=allow_override),
        )
        mutated = self._apply_machine_decision(session, decision)
    ...
    def check_satisfaction(
        ...
        *,
        ...
        if not active_skills:
            return RuntimeSkillCredentialSatisfactionOutcome(status="satisfied", credential_env={})
    ...
    async def submit_credential_value(
        ...
        *,
        ...
        self._credentials.save(actor_key, setup.skill, str(req["key"]), value)
        decision = decide_setup_action(self._snapshot(session), AdvanceSetupAction(actor_key=actor_key))
        mutated = self._apply_machine_decision(session, decision)
