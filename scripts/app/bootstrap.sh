...
# Install dev/test dependencies only when run standalone (not from setup_instance.sh).
if [ -z "${BOT_SETUP_RUNNING:-}" ]; then
    "$REPO_DIR/.venv/bin/pip" install -r "$REPO_DIR/requirements-dev.txt" -q
fi

# Only show next-steps when run standalone (not from setup_instance.sh)
if [ -z "${BOT_SETUP_RUNNING:-}" ]; then
    echo
    echo "Next: run ./scripts/host/setup_instance.sh to configure an instance."
fi
