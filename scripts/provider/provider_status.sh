...
if ! docker compose --project-directory . -f infra/compose/docker-compose.yml --profile bot --env-file .env.bot run --rm bot-provider; then
  exit 1
fi
echo "Success here does NOT prove the bot can start. For full app health (DB, config, Telegram) run: docker compose --project-directory . -f infra/compose/docker-compose.yml --profile bot --env-file .env.bot run --rm bot python -m app.main --doctor"
