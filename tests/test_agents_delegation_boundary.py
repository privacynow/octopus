...
async def test_delegation_approve_boundary_uses_explicit_runtime(monkeypatch):
    with fresh_env(
        ...
        assert submitted[0].parent_conversation_id == conversation_ref
...
async def test_delegation_approve_normalizes_registry_parent_conversation_id(monkeypatch):
    with fresh_env(
        config_overrides={
            "agent_mode": "registry",
            "agent_registries": (make_registry_connection(),),
        ...
        session = default_session(prov.name, prov.new_provider_state("reg:test"), "off")
        session["pending_delegation"] = {
            "conversation_ref": conversation_ref,
            "title": "Feature delegation",
            "tasks": [
        ...
        session_after = load_session_disk(data_dir, conversation_key, prov)
        pending = session_after.get("pending_delegation")
        assert len(submitted) == 1
        assert submitted[0].parent_conversation_id == "conv-1"
        assert pending is not None
        assert pending["status"] == "submitted"
        assert pending["tasks"][0]["status"] == "submitted"
        assert channel_egress.messages == [
