...
current Local/Shared Runtime contract and active roadmap phase.
"""
...
def test_architecture_preamble_states_current_local_runtime_baseline():
    """ARCHITECTURE preamble must summarize the current runtime baseline."""
    repo = Path(__file__).resolve().parent.parent
    text = (repo / "docs" / "ARCHITECTURE.md").read_text()
    preamble = text.split("\n---\n", 1)[0]
    assert "Current shipped baseline" in preamble
    assert "Phase 20" in preamble
    assert "Phase 21" in preamble
...
def test_architecture_deployment_section_matches_current_runtime_contract():
    """ARCHITECTURE deployment section must describe the current runtime contract."""
    repo = Path(__file__).resolve().parent.parent
    text = (repo / "docs" / "ARCHITECTURE.md").read_text()
    deployment = _section(text, "Deployment and dependencies")
    assert "Current runtime contract" in deployment
    assert "Local Runtime remains the default operator" in deployment
    assert "Shared Runtime is also a shipped runtime mode" in deployment
    assert "BOT_PROCESS_ROLE=all|webhook|worker" in deployment
    assert "BOT_DATABASE_URL" in deployment
    assert "./scripts/app/guided_start.sh" in deployment
    assert "./scripts/app/shared_start.sh" in deployment
    assert "infra/compose/docker-compose.shared.yml" in deployment
...
def test_status_current_snapshot_matches_current_phase_and_runtime():
    """STATUS Current Snapshot must reflect the current Phase 20/21 runtime truth."""
    repo = Path(__file__).resolve().parent.parent
    ...
    assert "M21A" in snapshot and "M21B" in snapshot
    assert "M21C" in snapshot
    assert "M21D" in snapshot
    assert "Shared Runtime requires `BOT_RUNTIME_MODE=shared` plus webhook mode" in snapshot
    assert "./scripts/app/shared_start.sh" in snapshot
    assert "infra/compose/docker-compose.shared.yml" in snapshot
