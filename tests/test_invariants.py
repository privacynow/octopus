...
designed to catch the class of bugs where one path drifts another
...
async def test_registry_search_does_not_block_event_loop():
    """Slow registry fetch must not prevent another command running."""
    import contextlib
    ...
    with fresh_data_dir() as data_dir:
        ...
        import app.workflows.runtime_skills.telegram as sc
        ...
        with patch("app.workflows.runtime_skills.telegram.asyncio.to_thread", side_effect=slow_to_thread):
...
async def test_resume_status_no_provider_name():
    ...
    with fresh_env(provider_name="claude") as (data_dir, cfg, prov):
        ...
        prov.run_results = [
            RunResult(text="first reply", provider_state_updates=ProviderStateRecord({"started": True})),
        ]
...
# =====================================================================
# INVARIANT: ClaudeProvider.run() sets resume_failed real stderr
#
...
@pytest.mark.asyncio
async def test_claude_provider_run_sets_resume_failed__stderr():
