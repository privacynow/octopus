...
async def test_concurrent_messages_each_claim_own_item():
    ...
    with fresh_env() as (data_dir, cfg, prov):
        ...
        # Serialized: admit then drain each so both get executed
        ...
        queued = conn.execute(
            "SELECT count(*) as n FROM work_items WHERE conversation_key = ? AND state = 'queued'",
            (_conv(chat_id),),
        ).fetchone()
        assert queued["n"] == 0, (
...
async def test_claim_for_update_blocked_by_existing_claimed_item():
    with fresh_env() as (data_dir, cfg, prov):
        ...
        work_queue.record_and_enqueue(data_dir, _event(300), _conv(chat_id), _actor(42), "message")
        work_queue.record_and_enqueue(data_dir, _event(301), _conv(chat_id), _actor(42), "message")
...
async def test_approval_callback_does_not_consume_stale_item():
    ...
    with fresh_env(config_overrides={"approval_mode": "on"}) as (data_dir, cfg, prov):
        ...
        work_queue.record_and_enqueue(data_dir, _event(500), _conv(chat_id), _actor(42), "message")
...
async def test_project_switch_waits_for_inflight_request():
    ...
    with fresh_env(config_overrides={
        ...
        # Send message then drain so worker runs it
...
async def test_preflight_and_execution_use_same_model():
    ...
    with fresh_env(config_overrides={
        ...
        # Send message then drain — triggers preflight in worker
...
async def test_live_command_blocked_by_worker_claimed_item():
    ...
    with fresh_env() as (data_dir, cfg, prov):
        ...
        cmd_items = conn.execute(
            "SELECT event_id, state FROM work_items "
            "WHERE conversation_key = ? AND event_id != ? ORDER BY event_id",
            (_conv(chat_id), _event(100)),
        ).fetchall()
...
async def test_failed_notice_delivery_marks_item_failed_via_worker_loop():
    ...
    with fresh_env() as (data_dir, cfg, prov):
        ...
        payload = serialize_inbound(event)
        _, item_id = work_queue.record_and_enqueue(
            data_dir, _event(1600), _conv(chat_id), _actor(42), "message", payload=payload,
        )
...
async def test_multiple_pending_recovery_items_each_addressable():
    ...
    with fresh_env() as (data_dir, cfg, prov):
        ...
        _, item_id_1 = work_queue.record_and_enqueue(
            data_dir, _event(1700), _conv(chat_id), _actor(42), "message",
...
async def test_replay_reclaim_blocked_by_existing_claimed_item():
    ...
    with fresh_env() as (data_dir, cfg, prov):
        ...
        _, item_id_recovery = work_queue.record_and_enqueue(
            data_dir, _event(1900), _conv(chat_id), _actor(42), "message",
        ...
        claimed_rows = conn.execute(
            "SELECT id FROM work_items WHERE conversation_key = ? AND state = 'claimed'",
            (_conv(chat_id),),
        ).fetchall()
...
async def test_replay_callback_blocked_by_claimed_item_answers_user():
    ...
    with fresh_env() as (data_dir, cfg, prov):
        ...
        payload = serialize_inbound(event)
        _, item_id_recovery = work_queue.record_and_enqueue(
            data_dir, _event(1910), _conv(chat_id), _actor(42), "message", payload=payload,
        )
        _, item_id_worker = work_queue.record_and_enqueue(
...
async def test_reclaim_distinguishes_gone_from_blocked():
    ...
    with fresh_env() as (data_dir, cfg, prov):
        ...
        _, item_id_recovery = work_queue.record_and_enqueue(
            data_dir, _event(2200), _conv(chat_id), _actor(42), "message",
        )
...
async def test_fresh_command_item_created_as_claimed():
    ...
    with fresh_env() as (data_dir, cfg, prov):
        ...
        items = conn.execute(
            "SELECT state, worker_id FROM work_items WHERE conversation_key = ?",
            (_conv(chat_id),),
        ).fetchall()
        assert len(items) == 1
...
async def test_worker_cannot_steal_handler_owned_item():
    ...
    with fresh_env() as (data_dir, cfg, prov):
        ...
        _, item_id = work_queue.record_and_enqueue(
            data_dir, _event(700), _conv(chat_id), _actor(42), "command",
...
async def test_claim_for_update_recognizes_pre_claimed():
    ...
    with fresh_env() as (data_dir, cfg, prov):
        ...
        _, item_id = work_queue.record_and_enqueue(
            data_dir, _event(900), _conv(chat_id), _actor(42), "command",
            worker_id=boot_id,
        )
        ...
        item2 = work_queue.claim_for_update(data_dir, _conv(chat_id), _event(900), "different-id")
...
async def test_complete_work_item_does_not_overwrite_terminal_state():
    ...
    with fresh_env() as (data_dir, cfg, prov):
        ...
        _, item_id = work_queue.record_and_enqueue(
            data_dir, _event(950), _conv(chat_id), _actor(42), "command",
        )
        # Complete it
...
async def test_preclaim_falls_back_to_queued_when_item_already_claimed():
    ...
    with fresh_env() as (data_dir, cfg, prov):
        ...
        work_queue.record_and_enqueue(data_dir, _event(1000), _conv(chat_id), _actor(42), "message")
        worker_item = work_queue.claim_next(data_dir, _conv(chat_id), "worker-1")
        assert worker_item is not None

        # New command arrives while worker holds the claim
        _, item_id = work_queue.record_and_enqueue(
            data_dir, _event(1001), _conv(chat_id), _actor(42), "command",
...
async def test_second_message_while_chat_has_runnable_queues_fifo():
    ...
    with fresh_env() as (data_dir, cfg, prov):
        ...
        msg_a = FakeMessage(chat=chat, text="first")
        upd_a = FakeUpdate(message=msg_a, user=user, chat=chat)
        await th.handle_message(upd_a, FakeContext())

        msg_b = FakeMessage(chat=chat, text="second")
        upd_b = FakeUpdate(message=msg_b, user=user, chat=chat)
